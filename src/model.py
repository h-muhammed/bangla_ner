...
class BasicBert(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.basicBert = BertForTokenClassification.from_pretrained('bert-base-cased', num_labels=self.opt.num_labels)
    
    def forward(self, input_id, mask, label):
        output = self.basicBert(input_ids=input_id, attention_mask=mask, labels=label, return_dict=False)
